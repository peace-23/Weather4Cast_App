    ...
    .then((weather) => {
      ...
      for (let day of weather.list) {
        ...
        innerHTML += `
          ...
          day.weather[0].icon
        }@2x.png' alt="" height="70%"></p>
                    <p>Wind at ${day.wind.speed.toFixed(
                      0
                    )} mph out of the ${windDirection(day.wind.deg)}</p>
                    <p>Sunrise: ${niceTime(
                      weather.city.sunrise,
                      0
                    )} | Sunset: ${niceTime(weather.city.sunset, 0)}</p>
...
function niceTime(dateTime, offset) {
  ...
  time =
    time.substring(0, time.lastIndexOf(":")) + time.substring(time.length - 3);
  return time;
}
